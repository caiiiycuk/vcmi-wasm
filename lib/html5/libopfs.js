...
function writeToHandle(accessHandle, offset, data, timestamp, mode) {
...
const onmessage = async (event) => {
  ...
  function write(root, path, offset, contents, timestamp, mode, syncCall = false, useHandleCache  = false) {
    const isDir = (mode & 0o170000) === 0o040000;
    if (!isDir) { // file
      const data = contents ?? new Uint8Array(0);
      if (syncCall) {
        if (cachedHandlePath === root + path) {
          const written = writeToHandle(cachedHandle, offset, data, timestamp, mode);
      ...
      return openHandle(root, path, true).then((accessHandle) => {
        const written = writeToHandle(accessHandle, offset, data, timestamp, mode);
  ...
  try {
    switch (type) {
      case 'pwrite': {
        ...
        parts.sort();
        let written = 0;
        for (const { path, offset, contents } of parts) {
          let result = write(root, path, offset, contents, timestamp, mode, true);
          if (result === null) {
            result = await write(root, path, offset, contents, timestamp, mode, false, true);
          }
          written += result;
        }
        closeCachedHandle();
      ...
      case 'write': {
        ...
        const written = await write(root, path, offset, contents, timestamp, mode);
        postSuccess(type, requestId, { written });
      } break;
      case 'pread': {
...
self.onmessage = (event) => onmessage(event).catch(console.error);
      ...
      reconcile: (mount, src, dst, callback) => {
        ...
        const promise = (async () => {
          ...
          if (OPFS.syncFsOnProgress && dst.type === 'local') {
            OPFS.syncFsOnProgress(0);
          }
          const updateProgress = (text, i, total) => {
            let progress = Math.round(i * 100 / total);
            if (progress > reportedProgress) {
              reportedProgress = progress;
              if (OPFS.syncFsOnProgress && dst.type === 'local') {
                OPFS.syncFsOnProgress(reportedProgress);
              }
            }
          }
          ...
          for (let i = 0; i < create.length; i++) {
            const path = create[i];
            if (dst.type === 'local' && OPFS.syncFsRemoteWhitelist !== null && !OPFS.syncFsRemoteWhitelist.has(path.toLowerCase())) {
      ...
      opfsWrite: async (root, path, offset, contents) => {
        ...
        return new Promise((resolve, reject) => {
          OPFS.callWorker("write", { root, path, offset, contents, timestamp: new Date(), mode: 0o100666 }, (err, result) => {
            if (err) {
              reject(err);
            } else {
              resolve(result);
          ...
          try {
            ...
            if (total > 0) {
              ...
              for (let i = 0; i < total; i++) {
                ...
                await OPFS.opfsWrite(directory, targetPath, 0, data);
                size += data.length;
