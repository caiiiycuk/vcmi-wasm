...
function getOpfsWorker() {
    if (globalThis.opfsWorker) {
        const w = globalThis.opfsWorker;
        if (!Number.isFinite(w.requestId)) {
            w.requestId = 0;
        }
        return w;
    }
...
export function opfsWrite(root, path, offset, contents) {
    return new Promise((resolve, reject) => {
        callWorker(
            "write",
            { root, path, offset, contents, timestamp: new Date(), mode: 0o100666 },
            (err, result) => {
                if (err) {
                    reject(err);
                } else {
